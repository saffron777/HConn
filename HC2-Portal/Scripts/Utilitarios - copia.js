...
function toogle() {
	...
	var arControles = OnClientLoadRadFileExplorer();
...
function OnClientLoadRadFileExplorer() {
	var controles = getRadControl(_fileExplorer)._element.control._gridData;
	var array = new Array();;
	for(i in controles)
		array[i] = controles[i]._name.substring(0, controles[i]._name.indexOf('.'));
