...
{
	///<summary>Clase BloquePresentadorLista.</summary>
    public class BloquePresentadorLista : PresentadorListaBase<Bloque>
    {
        ///<summary>Variable vista de la interfaz IBloqueLista.</summary>
        readonly IBloqueLista vista;
		
		///<summary>Constructor de la clase.</summary>
        ///<param name="vista">Variable tipo Interfaz de la vista.</param>
        public BloquePresentadorLista(IBloqueLista vista)
        {
            this.vista = vista;
        ...
        public void MostrarVista(string orden, int pagina, int tamañoPagina, IList<Infraestructura.Filtro> parametrosFiltro)
        {
			try
			{
	            BloqueRepositorio repositorio = new BloqueRepositorio(unidadDeTrabajo); 
	            IEnumerable<Bloque> datos = repositorio.ObtenerFiltrado(orden, pagina, tamañoPagina, parametrosFiltro);
        ...
        public void Eliminar(IList<string> ids)
        {
			try
			{
	            ...
	            foreach(string id in ids)
	            {
	                ...
	                Bloque _Bloque = repositorio.ObtenerPorId(idDesencriptado);
	                repositorio.EliminarLogico(_Bloque);
	            }
        ...
        {
            BloqueRepositorio repositorio = new BloqueRepositorio(this.unidadDeTrabajo);
        ...
        public void activarEliminado(IList<string> ids)
        {
            try
            {
                ...
                {
                    int idDesencriptado = int.Parse(System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(id.ToString())).Desencriptar());
                    Bloque _Aplicacion = repositorio.ObtenerPorId(idDesencriptado);
