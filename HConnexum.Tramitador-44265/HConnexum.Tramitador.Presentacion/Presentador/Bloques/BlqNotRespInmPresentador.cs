...
{
	public class BlqNotRespInmPresentador : BloquesPresentadorBase
	{
		readonly IBlqNotRespInm vista;

		///<summary>Constructor de la clase.</summary>
		///<param name="vista">Variable tipo Interfaz de la vista.</param>
		public BlqNotRespInmPresentador(IBlqNotRespInm vista)
		{
			this.vista = vista;
		}
		...
		public void MostrarVista()
		{
			try
			{
				...
				string mensajeMontoCompromiso = string.Empty;
				StringBuilder mensajeDocumentos = new StringBuilder();
				...
				HttpContext.Current.Trace.Warn("vista.ParametrosEntrada[\"IDSUSINTERMEDIARIO\"]: [" + vista.ParametrosEntrada[@"IDSUSINTERMEDIARIO"] + @"]");
						...
						if(!string.IsNullOrEmpty(vista.ParametrosEntrada[@"IDDIAGNOSTICO"]) && !string.IsNullOrEmpty(vista.ParametrosEntrada[@"IDPROCEDIMIENTO"]) && (vista.ParametrosEntrada[@"IDDIAGNOSTICO"] != @"0" && vista.ParametrosEntrada[@"IDPROCEDIMIENTO"] != @"0"))
						{
							if(tipoMov == @"ING" || (tipoMov == @"EGR" && tipoProc == @"AMBULATORIO"))
							{
								...
								using(DataSet ds = servicio.EjecutaStoredProcedure(@"pa_monto_cobertura_HC2", sConnectionString, parameterMontoCobertura))
									if(ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
										montoCobertura = string.IsNullOrWhiteSpace(ds.Tables[0].Rows[0][@"monto"].ToString()) ? 0 : double.Parse(ds.Tables[0].Rows[0][@"monto"].ToString());
								#endregion
				...
				{
					if(marcaAutomatica)
						mensajeMontoCompromiso = @"Monto Compromiso Aprobado: " + montoPresupuesto + @" Bs.";
					IEnumerable<string> totalDocumentos = documentosBase.Union(documentos);
					if(totalDocumentos.Count() > 0)
					{
						mensajeDocumentos.Append(@"Recuerde que debe consignar los siguientes Documentos:");
						foreach(string documento in totalDocumentos)
							mensajeDocumentos.AppendInNewLine(documento);
						mensajeDocumentos.Append(Environment.NewLine);
					}
					...
					#region Ejecucion de SP pa_movimiento_actual
					string idReclamo = string.Empty;
					HttpContext.Current.Trace.Warn("vista.ParametrosEntrada[\"IDRECLAMO\"]: [" + vista.ParametrosEntrada[@"IDRECLAMO"] + @"]");
					if(string.IsNullOrEmpty(vista.ParametrosEntrada[@"IDRECLAMO"]))
					{
						HttpContext.Current.Trace.Warn(@"Inicio pa_movimiento_actual_HC2");
						SqlParameter[] parameterMovimientoActual = new SqlParameter[1];
						parameterMovimientoActual[0] = new SqlParameter("@expediente", vista.ParametrosEntrada[@"IDEXPEDIENTEWEB"]);
						using(DataSet ds = servicio.EjecutaStoredProcedure(@"pa_movimiento_actual_SI_HC2", sConnectionString, parameterMovimientoActual))
                            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                            {
                                vista.ParametrosEntrada[@"IDRECLAMO"] = idReclamo = string.Empty + ds.Tables[0].Rows[0][@"id_reclamo"].ToString();
                                vista.PIdSupportIncident = string.Empty + ds.Tables[0].Rows[0][@"suport_incident_id"].ToString();
                               
                            }
						HttpContext.Current.Trace.Warn(@"idReclamo: [" + idReclamo + @"]");
						HttpContext.Current.Trace.Warn("vista.ParametrosEntrada[\"IDRECLAMO\"]: [" + vista.ParametrosEntrada[@"IDRECLAMO"] + @"]");
						HttpContext.Current.Trace.Warn(@"Fin pa_movimiento_actual_HC2");
					}
					else
						idReclamo = vista.ParametrosEntrada[@"IDRECLAMO"];
					#endregion
					if(!string.IsNullOrEmpty(vista.ParametrosEntrada[@"IDRECLAMO"]))
					{
						#region Ejecucion de SP pa_nuevo_movimiento
						SqlParameter[] parameterNuevoMov = new SqlParameter[19];
						parameterNuevoMov[0] = new SqlParameter("@reclamo", idReclamo);
						parameterNuevoMov[1] = new SqlParameter("@idexpediente", vista.ParametrosEntrada[@"IDEXPEDIENTEWEB"]);
						parameterNuevoMov[2] = new SqlParameter("@tipom", tipoMovCompleto);
						...
						using(DataSet ds = servicio.EjecutaStoredProcedure(@"pa_nuevo_movimiento_HC2", sConnectionString, parameterNuevoMov))
							if(ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
							{
								if((string.Empty + ds.Tables[0].Rows[0][@"idrecibido"]) != @"2")
								{
									mensaje.Clear();
									mensaje.Append(@"No puede volver a solicitar este movimiento, por favor espere respuesta de su caso.");
								}
							}
						#endregion
						vista.PIdReclamo = vista.ParametrosEntrada[@"IDRECLAMO"];
						vista.PEstatusMovimientoWeb = string.IsNullOrWhiteSpace(estatus) ? vista.ParametrosEntrada[@"ESTATUSMOVIMIENTOWEB"] : estatus;
						vista.PIndMvtoAutomatico = marcaAutomatica ? @"Activo" : @"NoActivo";
						vista.Mensaje = mensaje.ToString();
						vista.MostrarImprimir = estatusMov == @"OK";
					}
					else
						vista.Mensaje = @"No existe RECLAMO asociado.";
				}
				if(tipoMov == @"ANU")
				{
				...
				if(string.IsNullOrEmpty(mensaje.ToString()))
				{
					...
					else
					{
						if(tipoMovCompleto == "VERIFICACION" && estatusMovimientoWeb == "PENDIENTE")
							...
							valorMensajeHC2 = @"SOLINGHOSPAPROBAUT";
						else if(tipoMov == "INGRESO" && tipoProc == "HOSPITALIZACIÓN" && estatusMovimientoWeb == "RECHAZADO")
							valorMensajeHC2 = @"INGHOSPRECHAZ";
						else if(tipoMovCompleto == "EGRESO" && tipoProc == "AMBULATORIO" && estatusMovimientoWeb == "PENDIENTE")
					...
					if(!string.IsNullOrEmpty(mensajeMontoCompromiso))
					{
						mensaje.AppendInNewLine(mensajeMontoCompromiso);
						mensaje.Append(Environment.NewLine);
					}
					if(!string.IsNullOrEmpty(mensajeDocumentos.ToString()))
						mensaje.AppendInNewLine(mensajeDocumentos.ToString());
				}
				#endregion
				vista.Mensaje = mensaje.ToString();
			}
		...
		public void ValidarDatos()
		{
			...
			try
			{
				HttpContext.Current.Trace.Warn(@"Inicio ValidarDatos");
				HttpContext.Current.Trace.Warn("vista.ParametrosEntrada[\"IDSUSINTERMEDIARIO\"]:" + vista.ParametrosEntrada[@"IDSUSINTERMEDIARIO"] + @"]");
				HttpContext.Current.Trace.Warn("vista.PIdReclamo:" + vista.PIdReclamo + @"]");
				if(vista.ParametrosEntrada[@"TIPOMOV"].ToUpper() != @"ANULACIÓN" && string.IsNullOrEmpty(vista.PIdReclamo))
					errores.Append(@"No existe RECLAMO asociado.");
				HttpContext.Current.Trace.Warn(@"Fin ValidarDatos");
			}
			catch(Exception ex)
			{
				Errores.ManejarError(ex, "Error al Mostrar la data de la Aplicacion");
