...
namespace HConnexum.Tramitador.Presentacion.Presentador.Bloques
{
	...
	public class SeleccionAseguradoCAPresentador : BloquesPresentadorBase
	{
		...
		private void Auditoria(Exception pException, string pMensaje)
		{
			...
			catch(Exception ex)
			{
				...
				if(ex.InnerException != null)
					HttpContext.Current.Trace.Warn(TRAZA_CATEGORIA_ERROR, _mensaje + "Error en la capa origen: " + ex.InnerException.Message);
		...
		private bool EvaluaSolicitudAbierta(int pSuscriptorId, int pBeneficiarioId, int pProveedorId)
		{
			...
			{
				/// Información necesaria para realizar la traza de seguimiento.
				_mensaje = string.Format("{0}.EvaluaSolicitudAbierta(pSuscriptorId = {1}, pBeneficiarioId = {2}, pProveedorId = {3})", _NAMESPACE, pSuscriptorId, pBeneficiarioId, pProveedorId);
				_conexionCadena = ObtenerBDConexionCadena(pSuscriptorId);
				if(string.IsNullOrWhiteSpace(_conexionCadena))
					return false;
				_parametros.Clear();
		...
		readonly ISeleccionAseguradoCA Vista;
		#endregion
		...
		public SeleccionAseguradoCAPresentador(ISeleccionAseguradoCA vista)
		{
			try
			{
				/// Información necesaria para realizar la traza de seguimiento.
				_mensaje = string.Format("{0}.SeleccionAseguradoCAPresentador()", _NAMESPACE);
		...
		public void MostrarVista()
		{
			try
			{
				...
				string configValor = ConfigurationManager.AppSettings["CA_SolicitudOcurrenciaFechaMaximoDiasAtras"];
				this.Vista.IntermediarioPermiteAseguradoNoEncontrado =
					EvaluarIntermediarioCondicionAseguradoNoEncontrado(this.Vista.SuscriptorIntermediarioId, this.Vista.IntermediarioGestorId);
				this.Vista.SolicitudFechaValorMaximo = DateTime.Now.Date;
				this.Vista.SolicitudEmisionFechaValorMaximo = DateTime.Now.Date;
				this.Vista.SolicitudMenorNacimientoFechaValorMaximo = DateTime.Now.Date;
				...
				configValor = ConfigurationManager.AppSettings["ListaHospitalizacionDias_CA"];
				if(!string.IsNullOrWhiteSpace(configValor))
					this.Vista.ListaHospitalizacionDias = ObtenerListaValor(configValor);
			}
			catch(Exception ex)
			{
				Auditoria(ex, _mensaje);
        ...
        private string ObtenerFax(string idProveedor, string idIntermediario, int idsuscriptor) 
        {
            ...
            if (!string.IsNullOrEmpty(stringConexion))
            {
                ...
                if (!string.IsNullOrEmpty(idProveedor.ToString()) && !string.IsNullOrEmpty(idIntermediario.ToString()))
                {
                    ...
                    parametros[2] = new SqlParameter("@TipoServicio  ", "Carta");
		...
		/// <summary>Evalúa si el afiliado es válido para un intermediario determinado.</summary>
		/// <param name="pSuscriptorId">Identificador del suscriptor a consultar.</param>
		/// <param name="pCedula">Número de cédula de identidad del afiliado que se desea consultar.</param>
		/// <param name="pIntermediarioId">Identificador del intermediario a consultar.</param>
		public DataSet EvaluarAfiliadoValidez(int pSuscriptorId, string pCedula, int pIntermediarioId)
		{
			HttpContext.Current.Trace.Warn(@"Inicio EvaluarAfiliadoValidez");
			HttpContext.Current.Trace.Warn(@"pSuscriptorId: [" + pSuscriptorId + @"]");
			HttpContext.Current.Trace.Warn(@"pCedula: [" + pCedula + @"]");
			HttpContext.Current.Trace.Warn(@"pIntermediarioId: [" + pIntermediarioId + @"]");
			HttpContext.Current.Trace.Warn(@"sp: [pa_busqueda_poliza_Vig_HC2]");
			HttpContext.Current.Trace.Warn(@"Fin EvaluarAfiliadoValidez");

			_SP = "pa_busqueda_poliza_Vig_HC2";
			try
			{
				/// Información necesaria para realizar la traza de seguimiento.
				_mensaje = string.Format("{0}.EvaluarAfiliadoValidez(pSuscriptorId = {1}, pCedula = {2}, pIntermediarioId = {3})"
					, _NAMESPACE, pSuscriptorId, pCedula, pIntermediarioId);
				_conexionCadena = ObtenerBDConexionCadena(pSuscriptorId);
				if(string.IsNullOrWhiteSpace(_conexionCadena))
					return null;
				...
				_parametros.Add(_paramIntermediarioId);
				_ds = _connGestor.EjecutaStoredProcedure(_SP, _conexionCadena, _parametros.ToArray());
				this.Vista.AfiliadoPolizasRegistrosCantidad = _ds.Tables[0].Rows.Count;
				this.Vista.AfiliadoPolizasDatos = _ds.Tables[0];
				return _ds;
			}
			catch(Exception ex)
			{
		...
		public bool ObtenerPolizaGrupoFamiliar(int pSuscriptorId, int pPolizaId)
		{
			HttpContext.Current.Trace.Warn(@"Inicio ObtenerPolizaGrupoFamiliar");
			HttpContext.Current.Trace.Warn(@"pSuscriptorId: [" + pSuscriptorId + @"]");
			HttpContext.Current.Trace.Warn(@"pPolizaId: [" + pPolizaId + @"]");
			HttpContext.Current.Trace.Warn(@"sp: [pa_busca_beneficiario]");
			HttpContext.Current.Trace.Warn(@"Fin ObtenerPolizaGrupoFamiliar");

			_SP = "pa_busca_beneficiario";
			try
			{
				/// Información necesaria para realizar la traza de seguimiento.
				_mensaje = string.Format("{0}.ObtenerPolizaGrupoFamiliar(pSuscriptorId = {1}, pPolizaId = {1})", _NAMESPACE, pSuscriptorId, pPolizaId);
				_conexionCadena = ObtenerBDConexionCadena(pSuscriptorId);
				if(string.IsNullOrWhiteSpace(_conexionCadena))
					return false;
				_parametros.Clear();
		...
		/// <summary>Obtiene una lista de solicitudes abiertas de un beneficiario determinado con un proveedor en particular.</summary>
		public bool ObtenerSolicitudAbierta(int pSuscriptorId, string pTitularCedulaNro, string pBeneficiarioCedulaNro, int pProveedorId)
		{
			HttpContext.Current.Trace.Warn(@"Inicio ObtenerSolicitudAbierta");
			HttpContext.Current.Trace.Warn(@"pSuscriptorId: [" + pSuscriptorId + @"]");
			HttpContext.Current.Trace.Warn(@"pTitularCedulaNro: [" + pTitularCedulaNro + @"]");
			HttpContext.Current.Trace.Warn(@"pBeneficiarioCedulaNro: [" + pBeneficiarioCedulaNro + @"]");
			HttpContext.Current.Trace.Warn(@"pProveedorId: [" + pProveedorId + @"]");
			HttpContext.Current.Trace.Warn(@"sp: [pa_solicitudAbiertaWAF_ca_HC2]");

			_SP = "pa_solicitudAbiertaWAF_ca_HC2";
			try
			{
				/// Información necesaria para realizar la traza de seguimiento.
				_mensaje = string.Format("{0}.pa_solicitudAbiertaWAF_ca_HC2(pSuscriptorId = {1}, pPolizaId = {1})", _NAMESPACE, pSuscriptorId, pTitularCedulaNro, pBeneficiarioCedulaNro, pProveedorId);
				_conexionCadena = ObtenerBDConexionCadena(pSuscriptorId);
				if(string.IsNullOrWhiteSpace(_conexionCadena))
					return false;
				_parametros.Clear();
				_parametros.Add(new SqlParameter()
				{
					ParameterName = "@cedulaTit",
					SqlDbType = SqlDbType.VarChar,
					Size = 15,
					Value = pTitularCedulaNro
				});
				_parametros.Add(new SqlParameter()
				{
					ParameterName = "@cedulaAseg",
					SqlDbType = SqlDbType.VarChar,
					Size = 15,
					Value = pBeneficiarioCedulaNro
				});
				_parametros.Add(new SqlParameter()
				{
					ParameterName = "@clinica",
			...
			HttpContext.Current.Trace.Warn(@"Fin ObtenerSolicitudAbierta");
			return true;
		}
		...
		public void ValidarDatos()
		{
			try
			{
				...
				{
					this.Vista.Errores = MostrarMensaje(TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_RegistroSolicitudRequerido);
					return;
				}
				if(this.Vista.BeneficiarioExiste && !this.Vista.PanelAfiliadoPolizaVisible)
				{
					this.Vista.Errores = MostrarMensaje(TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_ConsultaAfiliadoRequerida);
					return;
				}
				if(this.Vista.BeneficiarioExiste && !this.Vista.PanelPolizaGrupoFamiliarVisible)
				{
					this.Vista.Errores = MostrarMensaje(TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_ConsultaGrupoFamiliarRequerida);
					return;
				}
				...
				if(this.Vista.TitularNacimientoFecha >= DateTime.Now.Date.AddYears(-18))
				{
					this.Vista.Errores = MostrarMensaje(TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_TitularNoMayorDeEdad);
					return;
				}
				if(ObtenerSolicitudAbierta(this.Vista.SuscriptorIntermediarioId, this.Vista.TitularCedulaNumero, this.Vista.BeneficiarioCedulaNumero, this.Vista.ProveedorGestorId))
				{
					this.Vista.Errores = MostrarMensaje(TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_SolicitudAbierta);
					return;
				}
				if(Vista.BeneficiarioParentesco.ToUpper() == "HIJO")
				{
					...
					{
						this.Vista.Errores = MostrarMensaje(TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_EdadIgualFechaNacimiento);
						return;
					}
				}
			...
			try
			{
				...
				switch(pTipo)
				{
					...
					case TiposMensaje.Validacion_CampoRequerido:
						...
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_Parentesco_Recaudos:
						...
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_SolicitudAbierta:
						result = "Ya existe una solicitud pendiente para este paciente.";
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_ConsultaAfiliadoRequerida:
						result = "Debe consultar un afiliado.";
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_ConsultaGrupoFamiliarRequerida:
						result = "Debe consultar el grupo familiar asociado al afiliado.";
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_RegistroSolicitudRequerido:
						result = "Debe registrar los datos de la solicitud.";
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_AfiliadoNoExiste:
                        result = "Si el paciente es beneficiario de la póliza, realice la búsqueda por la cédula del titular.";
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_TitularNoMayorDeEdad:
						result = "El titular debe ser mayor de edad.";
						break;
					case TiposMensaje.Informacion_Servicio_Carta_Aval_Verificacion_EdadIgualFechaNacimiento:
