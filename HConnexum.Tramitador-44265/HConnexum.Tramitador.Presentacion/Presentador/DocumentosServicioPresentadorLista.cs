...
{
	///<summary>Clase DocumentosServicioPresentadorLista.</summary>
    public class DocumentosServicioPresentadorLista : PresentadorListaBase<DocumentosServicio>
    {
		///<summary>Variable vista de la interfaz IDocumentosServicioLista.</summary>
        IDocumentosServicioLista vista;
		
		///<summary>Constructor de la clase.</summary>
        ///<param name="vista">Variable tipo Interfaz de la vista.</param>
        public DocumentosServicioPresentadorLista(IDocumentosServicioLista vista)
        {
            this.vista = vista;
        ...
        public void MostrarVista(string orden, int pagina, int tamañoPagina, IList<Infraestructura.Filtro> parametrosFiltro)
        {
			try
			{
	            DocumentosServicioRepositorio repositorio = new DocumentosServicioRepositorio(unidadDeTrabajo); 
	            IEnumerable<DocumentosServicioDTO> datos = repositorio.ObtenerDTONombres(orden, pagina, tamañoPagina, parametrosFiltro, vista.IdFlujoServicio);
        ...
        public void Eliminar(IList<string> ids)
        {
			try
			{
	            ...
	            foreach(string id in ids)
	            {
	                ...
	                DocumentosServicio _DocumentosServicio = repositorio.ObtenerPorId(idDesencriptado);
	                if(!repositorio.EliminarLogico(_DocumentosServicio, _DocumentosServicio.Id))
                    	this.vista.Errores = "Ya Existen registros asociados a esta  _" + _DocumentosServicio + "";
	            }
        ...
        {
            DocumentosServicioRepositorio repositorio = new DocumentosServicioRepositorio(this.unidadDeTrabajo);
        ...
        public void activarEliminado(IList<string> ids)
        {
            try
            {
                ...
                {
                    int idDesencriptado = int.Parse(System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(id.ToString())).Desencriptar());
                    DocumentosServicio _Aplicacion = repositorio.ObtenerPorId(idDesencriptado);
