...
{
	///<summary>Clase ParametrosAgendaPresentadorLista.</summary>
    public class ParametrosAgendaPresentadorLista : PresentadorListaBase<ParametrosAgenda>
    {
		///<summary>Variable vista de la interfaz IParametrosAgendaLista.</summary>
        IParametrosAgendaLista vista;
		
		///<summary>Constructor de la clase.</summary>
        ///<param name="vista">Variable tipo Interfaz de la vista.</param>
        public ParametrosAgendaPresentadorLista(IParametrosAgendaLista vista)
        {
            this.vista = vista;
        ...
        public void MostrarVista(string orden, int pagina, int tamañoPagina, IList<Infraestructura.Filtro> parametrosFiltro)
        {
			try
			{
	            ParametrosAgendaRepositorio repositorio = new ParametrosAgendaRepositorio(unidadDeTrabajo); 
	            //IEnumerable<ParametrosAgenda> datos = repositorio.ObtenerFiltrado(orden, pagina, tamañoPagina, parametrosFiltro);
                IEnumerable<ParametrosAgendaDTO> datos = repositorio.ObtenerDTO(orden, pagina, tamañoPagina, parametrosFiltro, vista.IdPasos);
        ...
        public void Eliminar(IList<string> ids)
        {
			try
			{
	            ...
	            foreach(string id in ids)
	            {
	                ...
	                ParametrosAgenda _ParametrosAgenda = repositorio.ObtenerPorId(idDesencriptado);
	                repositorio.EliminarLogico(_ParametrosAgenda);
	            }
        ...
        {
            ParametrosAgendaRepositorio repositorio = new ParametrosAgendaRepositorio(this.unidadDeTrabajo);
        ...
        public void activarEliminado(IList<string> ids)
        {
            try
            {
                ...
                {
                    int idDesencriptado = int.Parse(System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(id.ToString())).Desencriptar());
                    ParametrosAgenda _Aplicacion = repositorio.ObtenerPorId(idDesencriptado);
