...
{
	///<summary>Clase ServicioSucursalPresentadorLista.</summary>
    public class ServicioSucursalPresentadorLista : PresentadorListaBase<ServicioSucursal>
    {
		///<summary>Variable vista de la interfaz IServicioSucursalLista.</summary>
        IServicioSucursalLista vista;
		
		///<summary>Constructor de la clase.</summary>
        ///<param name="vista">Variable tipo Interfaz de la vista.</param>
        public ServicioSucursalPresentadorLista(IServicioSucursalLista vista)
        {
            this.vista = vista;
        ...
        public void MostrarVista(string orden, int pagina, int tamañoPagina, IList<Infraestructura.Filtro> parametrosFiltro, int idFlujoservicio)
        {
			try
			{
	            ServicioSucursalRepositorio repositorio = new ServicioSucursalRepositorio(unidadDeTrabajo); 
	            IEnumerable<ServicioSucursalDTO> datos = repositorio.ObtenerDTO(orden, pagina, tamañoPagina, parametrosFiltro,idFlujoservicio);
        ...
        public void Eliminar(IList<string> ids)
        {
			try
			{
	            ...
	            foreach(string id in ids)
	            {
	                ...
	                ServicioSucursal _ServicioSucursal = repositorio.ObtenerPorId(idDesencriptado);
	                if(!repositorio.EliminarLogico(_ServicioSucursal, _ServicioSucursal.Id))
                    	this.vista.Errores = "Ya Existen registros asociados a esta  _" + _ServicioSucursal + "";
	            }
        ...
        {
            ServicioSucursalRepositorio repositorio = new ServicioSucursalRepositorio(this.unidadDeTrabajo);
        ...
        public void activarEliminado(IList<string> ids)
        {
            try
            {
                ...
                {
                    int idDesencriptado = int.Parse(System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(id.ToString())).Desencriptar());
                    ServicioSucursal _Aplicacion = repositorio.ObtenerPorId(idDesencriptado);
