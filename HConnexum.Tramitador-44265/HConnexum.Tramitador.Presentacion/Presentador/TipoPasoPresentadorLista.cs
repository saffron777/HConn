...
{
	///<summary>Clase TipoPasoPresentadorLista.</summary>
    public class TipoPasoPresentadorLista : PresentadorListaBase<TipoPaso>
    {
        ///<summary>Variable vista de la interfaz ITipoPasoLista.</summary>
        readonly ITipoPasoLista vista;
		
		///<summary>Constructor de la clase.</summary>
        ///<param name="vista">Variable tipo Interfaz de la vista.</param>
        public TipoPasoPresentadorLista(ITipoPasoLista vista)
        {
            this.vista = vista;
        ...
        public void MostrarVista(string orden, int pagina, int tamañoPagina, IList<Infraestructura.Filtro> parametrosFiltro)
        {
			try
			{
	            TipoPasoRepositorio repositorio = new TipoPasoRepositorio(unidadDeTrabajo); 
	            IEnumerable<TipoPaso> datos = repositorio.ObtenerFiltrado(orden, pagina, tamañoPagina, parametrosFiltro);
        ...
        public void Eliminar(IList<string> ids)
        {
			try
			{
	            ...
	            foreach(string id in ids)
	            {
	                ...
	                TipoPaso _TipoPaso = repositorio.ObtenerPorId(idDesencriptado);
	                repositorio.EliminarLogico(_TipoPaso);
	            }
        ...
        {
            TipoPasoRepositorio repositorio = new TipoPasoRepositorio(this.unidadDeTrabajo);
        ...
        public void activarEliminado(IList<string> ids)
        {
            try
            {
                ...
                {
                    int idDesencriptado = int.Parse(System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(id.ToString())).Desencriptar());
                    TipoPaso _Aplicacion = repositorio.ObtenerPorId(idDesencriptado);
