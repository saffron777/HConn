...
{
	public partial class ListadoMovimientosCA : UserControlListaBase, IListadoMovimientosCA
	{
		#region "Variables"
		private string configClaveExcepcionMensaje = "MensajeExcepcion";
		ListadoMovimientosCAPresentador presentador;
		#endregion
		...
		protected void Page_Init(object sender, EventArgs e)
		{
			try
			{
				...
				this.presentador = new ListadoMovimientosCAPresentador(this);
				Orden = "Movimiento";
			}
			catch(Exception ex)
			{
		...
		protected void RadGridMaster_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
		{
			try
			{
				this.presentador.MostrarVista(Orden);
			}
			catch(Exception ex)
			{
				HConnexum.Infraestructura.Errores.ManejarError(ex, @"Error al Mostrar la data de la Aplicacion");
