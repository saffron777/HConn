...
{
	public partial class SolicitudNuevoMovimiento_Ing : UserControlBase, ISolicitudNuevoMovimiento
	{
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			try
			{
				...
				#region Inicializacion de Validadores
				TargetInput tiNumDiasHosp = null;
				if(RadInputManager1 != null && RadInputManager1.InputSettings.Count > 0)
				{
					...
					if(txtSettings.TargetControls != null && txtSettings.TargetControls.Count > 0)
						tiNumDiasHosp = txtSettings.TargetControls.FindTargetInputById("NumDiasHosp");
				}
				#region GUI
				if(tiNumDiasHosp != null)
					tiNumDiasHosp.Enabled = true;
				if(NomTipoMov.SelectedValue == presentador.MOVIMIENTO_INGRESO_AMBULATORIO_ID)
					if(tiNumDiasHosp != null)
						tiNumDiasHosp.Enabled = false;
				#endregion
			}
			catch(Exception ex)
			{
		...
		protected void NomTipoMov_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
		{
			...
			try
			{
				mostrar = (NomTipoMov.SelectedValue != presentador.MOVIMIENTO_INGRESO_ANULACION_ID);
				...
				if(NomTipoMov.SelectedValue == presentador.MOVIMIENTO_INGRESO_AMBULATORIO_ID)
				{
					...
					this.TipoCaso.Value = presentador.CASO_TIPO_AMBULATORIO_NOMBRE;
					this.NumDiasHosp.Text = string.Empty;
					this.NumDiasHosp.Enabled = false;
				}
				else if(NomTipoMov.SelectedValue == presentador.MOVIMIENTO_INGRESO_HOSPITALARIO_ID)
				{
					ULTIMOTIPOMOV.Value = ParametrosEntrada[@"TIPOMOV"];
					this.TipoMov.Value = "INGRESO";
					this.TipoCaso.Value = presentador.CASO_TIPO_HOSPITALARIO_NOMBRE;
					this.NumDiasHosp.Enabled = true;
				}
				else if(NomTipoMov.SelectedValue == presentador.MOVIMIENTO_INGRESO_ANULACION_ID)
				{
					ULTIMOTIPOMOV.Value = ParametrosEntrada[@"TIPOMOV"];
