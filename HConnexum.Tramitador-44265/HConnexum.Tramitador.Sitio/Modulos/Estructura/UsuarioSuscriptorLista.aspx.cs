...
namespace HConnexum.Tramitador.Sitio.Modulos.Estructura
{
    ...
    public partial class UsuarioSuscriptorLista : PaginaListaBase, IUsuariosSucriptor
    {
		///<summary>Variable presentador SuscriptorPresentadorLista.</summary>
        UsuariosSucriptorPresentador presentador; int IdUsuarioSuscriptror=0;    
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
		 	try
            {
                ...
                presentador = new UsuariosSucriptorPresentador(this);
                if (!string.IsNullOrEmpty(this.Request.QueryString["IdUsuarioAsignado"].ToString()))
                    IdUsuarioSuscriptror = int.Parse(this.Request.QueryString["IdUsuarioAsignado"].ToString());
                    Orden = "Id";
			}
            catch (Exception ex)
            {
		...
		protected void RadGridMaster_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
		{
			try
			{
                presentador.MostrarVista(Orden, NumeroPagina, TamanoPagina, ParametrosFiltro, UsuarioActual.IdUsuarioSuscriptorSeleccionado,IdUsuarioSuscriptror);
			}
            catch (Exception ex)
            {
                HConnexum.Infraestructura.Errores.ManejarError(ex, "Error al Mostrar la data de la Aplicacion");
        ...
        public IEnumerable<UsuariosSucriptorDTO> Datos
        {
            set
            {
