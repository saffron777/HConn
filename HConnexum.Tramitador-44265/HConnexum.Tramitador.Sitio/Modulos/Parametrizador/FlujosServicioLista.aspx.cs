...
{
	///<summary>Clase FlujosServicioLista.</summary>
    public partial class FlujosServicioLista : PaginaListaBase, IFlujosServicioLista
    {
		///<summary>Variable presentador FlujosServicioPresentadorLista.</summary>
        FlujosServicioPresentadorLista presentador;
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
		 	try
            {
                ...
                presentador = new FlujosServicioPresentadorLista(this);
	            Orden = "Id";
			}
            catch (Exception ex)
            {
		...
		protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
        {
			try
            {
	           	if(!string.IsNullOrEmpty(e.Argument))
	            	presentador.EliminarRegistroTomadoAjax(e.Argument);
				this.RadGridMaster.Rebind();
			}
            catch (Exception ex)
            {
			...
			try
			{
	            switch(e.CommandName)
	            {
                    ...
                    case "Copiar":
                        ...
                        {
                            if (item.Selected)
                                presentador.CopiarImportar(item.OwnerTableView.DataKeyValues[item.ItemIndex]["IdEncriptado"].ToString(), "Copiar");
                        }
                        break;
                    case "Importar":
                         foreach (GridDataItem item in RadGridMaster.Items)
                        {
                            if (item.Selected)
                                presentador.CopiarImportar(item.OwnerTableView.DataKeyValues[item.ItemIndex]["IdEncriptado"].ToString(), "Importar");
                        }
                        break;
        ...
        public IEnumerable<FlujosServicioDTO> Datos
        {
            set
            {
