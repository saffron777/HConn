...
namespace HConnexum.Tramitador.Sitio.Modulos.Parametrizador
{
    ...
    public partial class MensajesMetodosDestinatarioDetalle : PaginaDetalleBase, IMensajesMetodosDestinatarioDetalle
    {
        #region "Variables Locales"
        ///<summary>Variable presentador MensajesMetodosDestinatarioPresentadorDetalle.</summary>
        MensajesMetodosDestinatarioPresentadorDetalle presentador;
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            try
            {
                ...
                presentador = new MensajesMetodosDestinatarioPresentadorDetalle(this);
        ...
        {
            if (this.Telefono.NumeroTlfCompletoGuion.ToString() != "")
            {
                string Texto = Telefono.NumeroTlfCompleto.ToString() + "; ";
                string TextoFinal = this.MultilineCounter.Text;
                TextoFinal = TextoFinal + Texto;
                MultilineCounter.Text = TextoFinal;
                Telefono.CodigoArea = " ";
                Telefono.Numero = " ";
            }
        }
        ...
        {
            get { return MultilineCounter.Text; }
            set { MultilineCounter.Text = value; }
        }
        ...
        public IList<MensajesMetodosDestinatarioDTO> ValorBusqueda
        {
            get
            {
                return (IList<MensajesMetodosDestinatarioDTO>)ObjetoSession.LeerVariableSession("TAOMensajesMetodosDestinatarioDTO"); 
            }
            set
            {
                ObjetoSession.GrabarVariableSession("TAOMensajesMetodosDestinatarioDTO", value);
            }
        }

        public Dictionary<int, string> CasosMovimientosAsociados
        {
            get
            {
                Dictionary<int, string> tempDic = new Dictionary<int, string>();
                for (int i = 0; i < this.rlbListBoxCasosMovimientosAsociados.Items.Count; i++)
                    tempDic.Add(int.Parse(this.rlbListBoxCasosMovimientosAsociados.Items[i].Value), this.rlbListBoxCasosMovimientosAsociados.Items[i].Text);
                return tempDic;
            }
        }

        public DataTable ListBoxCasosMovimientosNoAsociados
        {
            set
            {
        ...
        public IList<MensajesMetodosDestinatarioDTO> ListBoxCasosMovimientosAsociados
        {
            set
            {
                this.ValorBusqueda = value;
                this.rlbListBoxCasosMovimientosAsociados.DataSource = value;
                this.rlbListBoxCasosMovimientosAsociados.DataBind();
            }
        }
