...
namespace HConnexum.Tramitador.Sitio.Modulos.Tracking
{
	...
	public partial class MovimientoCasoLista : PaginaListaBase, IMovimientoCasoLista
	{
		public int _Caso;
		string estatusCaso;
		public string VariableEncriptadaFCF = HttpServerUtility.UrlTokenEncode(Encoding.UTF8.GetBytes(("FCF").Encriptar()));
		public string VariableEncriptadaF = HttpServerUtility.UrlTokenEncode(Encoding.UTF8.GetBytes(("F").Encriptar()));
		public string bInterventor = HttpServerUtility.UrlTokenEncode(Encoding.UTF8.GetBytes(("true").Encriptar()));

		///<summary>Variable presentador MovimientoPresentadorLista.</summary>
		MovimientoCasoPresentadorLista presentador;
		...
		protected void Page_Init(object sender, EventArgs e)
		{
			try
			{
				...
				presentador = new MovimientoCasoPresentadorLista(this);
				Orden = "Id";
				_Caso = int.Parse(System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(this.Request.QueryString["idcaso"])).Desencriptar());
				estatusCaso = System.Text.Encoding.UTF8.GetString(HttpServerUtility.UrlTokenDecode(this.Request.QueryString["EstatusCaso"])).Desencriptar();
			}
			catch(Exception ex)
			{
		...
		protected void RadGridMaster_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
		{
			try
			{
				this.presentador.MostrarVista(this.Orden, this.NumeroPagina, this.TamanoPagina, this.ParametrosFiltro, _Caso);
			}
			catch(Exception ex)
			{
				HConnexum.Infraestructura.Errores.ManejarError(ex, @"Error al Mostrar la data de la Aplicacion");
